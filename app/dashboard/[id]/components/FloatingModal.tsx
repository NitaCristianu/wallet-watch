...
function FloatingModal({
  ...
  return (
    <motion.div
      className="bg-gray-100/50 h-screen w-screen max-md:w-screen md:h-[97%] backdrop-blur-xl top-1/2 -translate-y-1/2 rounded-2xl md:right-5 fixed flex flex-col justify-center overflow-hidden max-sm:left-0"
    ...
    >
      ...
      {action ? (
        <AnimatePresence>
          {step == 0 ? (
            <Step1 action={action} setstep={setstep} setpreset={stepreset} />
          ) : null}
          {step == 1 ? (
            <Step2
              type={action}
